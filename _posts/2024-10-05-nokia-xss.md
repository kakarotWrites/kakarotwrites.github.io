...
I started with passive recon to gather subdomains and IPs associated with Nokia. For this, I used the following tools: **<span style="color: #2980B9;">Sublist3r</span>**, **<span style="color: #2980B9;">Amass</span>**, and **<span style="color: #2980B9;">Subfinder</span>** to gather a large list of Nokia subdomains.
...
Many subdomains returned HTTP 200 with Cisco VPN or WebVPN portals exposed. To test for the vulnerability, we need to send a POST request to ``/+CSCOE+/saml/sp/acs?tgname=a`` with the ``SAMLResponse`` parameter set to a payload that triggers the XSS vulnerability.
