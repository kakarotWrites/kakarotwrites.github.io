...
```
<p>Many subdomains returned HTTP 200 with Cisco VPN or WebVPN portals exposed.</p>

<h2 style="color: #2E86C1;">Testing for CVE-2020-3580:</h2>

<p>To test for the vulnerability, we need to send a POST request to <code>/+CSCOE+/saml/sp/acs?tgname=a</code> with the SAMLResponse parameter set to a payload that triggers the XSS vulnerability. 
The payload is: <code>'><svg/onload=alert('xss')></code></p>
...
<p>I wrote a Bash script to check each of the subdomains for this vulnerability. Many of the subdomains were found to be vulnerable, and I reported them to Nokia through their responsible disclosure policy.</p>
