...
<p>To test for the vulnerability, we need to send a POST request to <code>/+CSCOE+/saml/sp/acs?tgname=a</code> with the SAMLResponse parameter set to a payload that triggers the XSS vulnerability. The payload is: <code>'><svg/onload=alert('xss')></code></p>
